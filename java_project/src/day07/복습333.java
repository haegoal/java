...
public class 복습333 {
	...
	public static void main(String[] args) {
		...
		int cnt = 0;
		boolean login = false;
		int loginIndex = -1;  // 배열은 0부터이므로 구분하기 어려워서 기본값을 -1로 설정 
			...
			if(menu == 1) {
				if(login) {
					...
					while(true) {
						...
						System.out.print("선택>");
						int menu2 = sc.nextInt(); // 믈픔 구매를 위한 메뉴 변수 선언 후 입력받음
						
						if(menu2 == 1) {
							if(balance[loginIndex] >=3000) {
								System.out.println("자장면 구매");
							...
							System.out.print("충전금액은 >");
							int bb = sc.nextInt();
							balance[loginIndex] += bb;
					...
					for(int i=0; i<cnt; i++) {
						if(loginId.equals(id[i])){
							...
							if(loginPw.equals(pw[i])) {
								...
								login = true;
								loginIndex = i; // 로그인한 아이디의 index를 넣음
								break;
							}else {
