...
public class Ex01_drink {
	...
	public static void main(String[] args) {
		...
		// 자판기 프로그램
		// 음료수에 해당하는 숫자를 받아 그 금액을 차감시키고 만약 잔액이 부족하면 잔액 부족으로 출력
		// 4를 선택한 경우는 충전으로 잔액 변수에 입력숫자를 더함  
		// 0~4 이외의 숫자 입력시 입력 오류 출력
		// 0이면 프로그램 종료
		...
		while (run) { // run이 true이므로 무한반복함
			...
			case 1:
				...
				if (balance < (1000)) { // 콜라 금액이 1000원이므로 잔액이 부족한지 조건을 지정
					System.out.println("잔액 부족!!"); // 콜라 금액보다 작으면 '잔액부족'이라고 출력
				} else { // 아니면 (금액이 충분하면) 금액 만큼 빼기
					balance = balance - 1000; // balance -=1000와 동일
//					System.out.println("잔액은 " + balance);
			...
			case 2:
				...
				if (balance < 1200) { // 사이다 금액이 1200원이므로 잔액이 부족한지 조건을 지정
					System.out.println("잔액 부족!!"); // 금액이 부족하면 잔액 부족 출력
				} else {
					balance = balance - 1200; // 사이다 금액만큼 빼기
//					System.out.println("잔액은 " + balance);
				}
				break;
			...
			case 3:
				...
				if (balance < 1500) { // 우유 금액이 1500원이므로 잔액이 부족한지 조건을 지정
					System.out.println("잔액 부족!!"); // 금액이 부족하면 잔액 부족 출력
				} else {
					balance = balance - 1500; // 우유 금액만큼 빼기
//					System.out.println("잔액은 " + balance);
				}
				break;
...
//	} else if (menu == 0) {
//		run = false; //break;
//		// run = false;는 탈출 명령이 아니므로 while를 수행 후 조건 물어봐서 탈출
//		// break는 바로 조건문 탈출
//	} else  {
